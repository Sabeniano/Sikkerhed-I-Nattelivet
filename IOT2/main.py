...
@app.route("/kort")
def kort():
    all_cords = Bruger.koordinater()
    latitude, longitude = 55.69178, 12.55388
    m = folium.Map(location=[latitude, longitude], zoom_start=13)
    for cords_from_db in all_cords:
        latitude_from_db, longitude_from_db = cords_from_db
        folium.Marker(location=[latitude_from_db, longitude_from_db], popup='Lokation').add_to(m)
    return render_template('kort.html', map=m._repr_html_())
